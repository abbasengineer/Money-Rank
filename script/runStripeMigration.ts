// Script to run Stripe fields migration
...
if (!DATABASE_URL) {
  ...
  console.error('Or pass as argument: tsx script/runStripeMigration.ts <database-url>');
  process.exit(1);
}
...
async function runMigration() {
  ...
  try {
    ...
    const migrationPath = join(projectRoot, 'script', 'migrate_add_stripe_fields.sql');
